...
def getUrl(html):
    ...
    reg = r'(?:href|HREF)="?((?:papers/eccv_2022/papers_ECCV/papers/)?.+?\.pdf)'
    ...
    url_list = [url.split('/')[-1] for url in url_list if '2022' in url and 'supp' not in url]
    return(url_list)
...
root_url = 'https://www.ecva.net/papers/eccv_2022/papers_ECCV/papers/'  #下载地址中相同的部分
...
content_list = [c for c in content_list if '2022' in c]
file_name_list = [c.split('</a>')[1].split("\\'")[1].split('/')[-1] for c in content_list if '2022' in c]
title_list = [c.split('</a>')[0].split('\\n')[-1] for c in content_list if '2022' in c]
assert len(title_list) == len(file_name_list)

root_dir = 'E:\Desktop\ECCV22'
