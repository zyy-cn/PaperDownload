...
def norm_title(title):
    ...
    title = title.replace('Redefining \\\\ensuremath{&lt;Creative-', 'Redefining -Creative- in Dictionary：Towards an Enhanced Semantic Understanding of Creative Generation')
    title = title.replace('\\\\ensuremath{&lt;SEG-', '-SEG- Token Works')
    return title
...
# all stuff we need
def getUrl(html):
    reg = r'(?:href|HREF)="?((?:/content/CVPR2025/papers/)?.+?\.pdf)'
...
root_url = '/content/CVPR2025/papers/'  #下载地址中相同的部分

raw_url = 'https://openaccess.thecvf.com/CVPR2025?day=all'
...
title_list = [title.split('}')[0] if title.split('}')[1][0] == ',' else title.split('}')[0]+title.split('}')[1] for title in content_list]
